...
const MovieEditAD = ({
  ...
  const [posterFile, setPosterFile] = useState(movie.poster || null);
  const [backgroundFile, setBackgroundFile] = useState(
    movie.background || null
  );
  const [moviename, setmoviename] = useState(movie.moviename || null);
  const [author, setauthor] = useState(movie.author || null);
  const [release_year, setrelease_year] = useState(movie.release_year || null);
  const [time, settime] = useState(movie.time || null);
  const [episodes, setepisodes] = useState(movie.episodes || null);
  const [movieurl, setmovieurl] = useState(movie.movieurl || null);
  const [moviedescribe, setmoviedescribe] = useState(
    movie.moviedescribe || null
  );
  const [moviesubname, setmoviesubname] = useState(movie.moviesubname || null);
  ...
  useEffect(() => {
    if (typedata && typedata.length > 0) {
      const initialSelectedTypes = typedata.map((type) => type.typeid);
      setSelectedTypes(initialSelectedTypes);
    }
    if (categorydata && categorydata.length > 0) {
      const initialSelectedCategory = categorydata[0].categoryid; // Ví dụ: chỉ lấy category đầu tiên
